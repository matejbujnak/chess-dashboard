...
export function RatingsOverview({ ratings }: Props) {
  ...
  return (
    <div>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-[#a0aec0]">Rating prehľad</h3>
        ...
        {sorted.map((r) => (
          <Card key={r.timeControl} className="flex flex-col gap-1 p-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-[#4a5568]">{r.label}</span>
            </div>
            <div className="text-2xl font-bold text-white">{r.current}</div>
            {r.best && r.best !== r.current && (
              <div className="text-xs text-[#10b981]">Najlepší: {r.best}</div>
            )}
            {r.total > 0 && (
              <div className="mt-1 space-y-0.5 text-xs text-[#a0aec0]">
                ...
                <div className="flex justify-between">
                  <span>Win%</span>
                  <span className="text-[#10b981]">{winRate(r.wins, r.losses, r.draws)}</span>
                </div>
              </div>
            )}
            {/* Win/Loss/Draw bar */}
            {r.total > 0 && (
              <div className="mt-2 flex h-1.5 overflow-hidden rounded-full">
                ...
                <div
                  className="bg-[#a0aec0]"
                  style={{ width: `${(r.draws / r.total) * 100}%` }}
                />
                <div
                  className="bg-[#ef4444]"
