...
export function ColorStats({ games }: Props) {
  ...
  const data = [
    {
      color: t.white,
      wins: white.filter((g) => g.result === "win").length,
      draws: white.filter((g) => g.result === "draw").length,
      losses: white.filter((g) => g.result === "loss").length,
    },
    {
      color: t.black,
      wins: black.filter((g) => g.result === "win").length,
      draws: black.filter((g) => g.result === "draw").length,
      losses: black.filter((g) => g.result === "loss").length,
    },
  ]

  const whiteWr = white.length > 0 ? ((data[0].wins / white.length) * 100).toFixed(0) : "0"
  const blackWr = black.length > 0 ? ((data[1].wins / black.length) * 100).toFixed(0) : "0"

  return (
    <Card>
      <CardHeader>
        <CardTitle>♔♚ {t.whiteVsBlack}</CardTitle>
